...
@login_required
def notifications():
    # ဝင်လာသမျှ Notification တွေကို ယူမယ်
    user_notifs = Notification.query.filter_by(user_id=current_user.id).order_by(Notification.id.desc()).all()
    
    # ဝင်လာတဲ့ Notification တွေကို ဖတ်ပြီးပြီလို့ အလိုအလျောက် မှတ်သားမယ် (Mark as read)
...
def click_notification(notif_id):
    ...
    if notif.user_id == current_user.id:
        # သက်ဆိုင်ရာ Post ဆီကို redirect လုပ်မယ်
